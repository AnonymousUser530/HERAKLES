...
def get_slurm_script(k):
    return f'''#!/bin/bash
#SBATCH --job-name=DLP_textcrafter_ablation_no_update_HL_gtt_to_pf_unsloth_test_generalisation_sr_epoch_{k}_seed_%a # job name
#SBATCH --time=03:00:00   # maximum execution time (HH:MM:SS)
#SBATCH --output=YOUR_FOLDER/slurm_logs/DLP_textcrafter_ablation_no_update_HL_gtt_to_pf_unsloth_test_generalisation_sr_epoch_{k}_seed_%a.out # output
#SBATCH --error=YOUR_FOLDER/slurm_logs/DLP_textcrafter_ablation_no_update_HL_gtt_to_pf_unsloth_test_generalisation_sr_epoch_{k}_seed_%a.err # err
...
srun slurm/launcher.sh \
                    rl_script_args.path=YOUR_FOLDER/DLP/test_hierarchical_agent_ablation_no_HL_update.py \
                    test_training_script.epoch_k={k} \
                    test_training_script.nbr_tests_hierarchical_agent=40 \
                    test_training_script.goal_space=["collect_wood","collect_2_woods","collect_3_woods","collect_4_woods","make_wood_sword","acquire_wood","install_table","create_wood_pickaxe"] \
                    test_training_script.test_name=generalisation \
                    ...
                    hydra.run.dir=YOUR_FOLDER/hydra_file/DLP_textcrafter_ablation_no_update_HL_gtt_to_pf_unsloth_test_generalisation_sr_epoch_{k}/seed_${{SLURM_ARRAY_TASK_ID}} \
...
n_runs = 0
for k in range(11):
    with open(f'YOUR_FOLDER/DLP/test_sr/DLP_textcrafter_ablation_no_update_HL_gtt_to_pf_unsloth_test_generalisation_sr_epoch_{k}.slurm', 'w') as f:
        f.write(get_slurm_script(k))
    subprocess.run(['sbatch', f'YOUR_FOLDER/DLP/test_sr/DLP_textcrafter_ablation_no_update_HL_gtt_to_pf_unsloth_test_generalisation_sr_epoch_{k}.slurm'])
    n_runs += 1
