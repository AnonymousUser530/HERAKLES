...
class Player(Object):
  ...
  def reset_to_achs_type(self, achs_type):
    self.achs_type = achs_type
    self.achievements = {name: 0 for name in constants.__dict__[f"{achs_type}_achievements"]}
  ...
  def update(self):
    ...
    if action not in ['sleep', 'do nothing', 'move left', 'move right', 'move up', 'move down', "do"]:
      act_on = action.split(' ', 1)[1]
    if action == 'do nothing':
      pass
    elif action.startswith('move '):
      self._move(action[len('move '):])

    elif action == 'do':
      if action == 'do' and obj:
        self._do_object(obj)
      elif action == 'do':
  ...
  def _move(self, direction):
    ...
    target = (self.pos[0] + self.facing[0], self.pos[1] + self.facing[1])
    material, obj = self.world[target]
    ach_name = f"go_to_{material}"
    if ach_name in self.achievements:
      self.achievements[ach_name] += 1
    if self.achs_type == "wsub":
      # check material right in front
      target = (self.pos[0] + self.facing[0], self.pos[1] + self.facing[1])
      material, obj = self.world[target]
      ach_name = f"goto_{material}"
      if ach_name in self.achievements:
        self.achievements[ach_name] += 1
      # check materials around
      nearby, _ = self.world.nearby(self.pos, 1)
      for m in nearby:
        ach_name = f"gonear_{m}"
        if ach_name in self.achievements:
          self.achievements[ach_name] += 1
  ...
  def _place(self, name, target, material):
    ...
    if self.placed_objects.get(name):
      self.placed_objects[name].append(target)
    else:
      self.placed_objects[name] = [target]
    if self.achs_type == "exp":
      nearby, _ = self.world.nearby(target, 1)
      for m in nearby:
        ach_name = f"place_{name}_{m}"
        if ach_name in self.achievements:
          self.achievements[ach_name] += 1
