...
OBJS = ['table', 'furnace']
MATS = ['wood', 'stone', 'coal', 'iron', 'diamond', 'drink', 'bush', 'cow', 'plant']
TOOLS = ['sword', 'pickaxe']
INSTRS = ['collect', 'make', 'place', 'chop', 'eat', 'go', *np.unique(np.array(list(SYNONYMS.values())).flatten())]
COUNT = [str(i) for i in range(2, 11)]
ENC_ORDER = INSTRS + OBJS + MATS + TOOLS + COUNT
"""
no dummy task in our settings
...
class Env(env.Env):
  ...
  def __init__(
      ...
      reward=True, length=1500, seed=None, train=True, reset_word_only_at_episode_termination=False, **kwargs):
    ...
    for ach in target_achievements:
      act = ach.split('_')[0]
      if act in ach and act in ["collect"]+SYNONYMS["collect"]:
          counts.append(10)
      elif act in ach and act in ["go"]+SYNONYMS["go"]:
          counts.append(1)
      else:
        counts.append(5)
    ...
    self.episode_termination = True
    self.reset_word_only_at_episode_termination = reset_word_only_at_episode_termination
    self.train = train
  ...
  def reset(self, goal=None):
    self._episode += 1
    if self.reset_word_only_at_episode_termination:
      if self.episode_termination:
        self._step = 0
    ...
    self.episode_termination = False
    self.task_steps = 0
  ...
  def step(self, action):
    ...
    self.episode_termination = done # self._length and self._step >= self._length
    done = done or self.task_done
    if self.reset_word_only_at_episode_termination:
      # reset the world when either you reach the max number of steps
      self.reset_world = self.episode_termination
    else:
      # reset the world when either you reach the max number of steps or complete your goal
  ...
  def _obs(self):
    return {
      'image': self.render(semantic=False),
      'task_enc': self._encode_task(),
    }
    ...
    for i, s in enumerate(original_list):
      if s.startswith('Dummy'):
        ...
        if parts[-1].isdigit():
            count = int(parts[-1])
            if parts[1]=="to":
              action = parts[0].capitalize() + " to"
              item_parts = parts[2:-1]
            else:
