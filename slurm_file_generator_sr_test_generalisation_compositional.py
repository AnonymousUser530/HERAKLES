...
def get_slurm_script(k, nbr_compo):
    return f'''#!/bin/bash
#SBATCH --job-name=DLP_textcrafter_gtt_to_pf_unsloth_ll_sr_update_256_lr_HL_1e5_test_generalisation_compo_{nbr_compo}_sr_epoch_{k}_seed_%a # job name
#SBATCH --time=01:45:00   # maximum execution time (HH:MM:SS)
#SBATCH --output=YOUR_FOLDER/slurm_logs/DLP_textcrafter_gtt_to_pf_unsloth_ll_sr_update_256_lr_HL_1e5_test_generalisation_compo_{nbr_compo}_sr_epoch_{k}_seed_%a.out # output
#SBATCH --error=YOUR_FOLDER/slurm_logs/DLP_textcrafter_gtt_to_pf_unsloth_ll_sr_update_256_lr_HL_1e5_test_generalisation_compo_{nbr_compo}_sr_epoch_{k}_seed_%a.err # err
...
srun slurm/launcher.sh \
                    rl_script_args.path=YOUR_FOLDER/DLP/test_hierarchical_agent_compositional.py \
                    test_training_script.epoch_k={k} \
                    test_training_script.nbr_tests_hierarchical_agent=50 \
                    test_training_script.test_generalisation.nbr_compositional_tasks={nbr_compo} \
                    test_training_script.test_name=generalisation_compositional{nbr_compo} \
                    ...
                    hydra.run.dir=YOUR_FOLDER/hydra_file/DLP_textcrafter_gtt_to_pf_unsloth_ll_sr_update_256_lr_HL_1e5_test_generalisation_compo_{nbr_compo}_sr_epoch_{k}/seed_${{SLURM_ARRAY_TASK_ID}} \
                    ...
                    rl_script_args.task_space=["collect_wood","place_table","make_wood_pickaxe","collect_stone","collect_coal","place_furnace"] \
...
n_runs = 0
for k in range(13):
    for nbr_compo in range(2, 5):
        with open(f'YOUR_FOLDER/DLP/test_sr/DLP_textcrafter_gtt_to_pf_unsloth_ll_sr_update_256_lr_HL_1e5_test_generalisation_compo_{nbr_compo}_sr_epoch_{k}.slurm', 'w') as f:
            f.write(get_slurm_script(k, nbr_compo))
        subprocess.run(['sbatch', f'YOUR_FOLDER/DLP/test_sr/DLP_textcrafter_gtt_to_pf_unsloth_ll_sr_update_256_lr_HL_1e5_test_generalisation_compo_{nbr_compo}_sr_epoch_{k}.slurm'])
        n_runs += 1


print("Total runs: " + str(n_runs))