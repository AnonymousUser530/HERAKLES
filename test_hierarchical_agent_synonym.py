...
def main(config_args):
    ...
    for name_env in task_space:
        ...
        print("\nTesting task: {}".format(name_env))
        splits = name_env.split("_")
        for act_s in SYNONYMS[splits[0]]:
            name_env_synonym = name_env.replace(splits[0], act_s)
            print("- Testing task: {}".format(name_env_synonym))
            successs_, lengths_n, interaction_lengths_n, subgoal_instructions_dict = test_hierarchical_agent_sr_from_reset_env(name_env=name_env_synonym,
                                                                                     seed=seed,
                                                                                     ...
                                                                                     device=device,
                                                                                     nbr_tests=config_args.test_training_script.nbr_tests_hierarchical_agent//len(SYNONYMS[splits[0]]),
                                                                                     rl_script_args=config_args.rl_script_args,
                                                                                     task_space=task_space,
                                                                                     action_space_hl=action_space_hl)
            if test_results["success_rate_hierarchical_agent_from_reset_env"].get(name_env) is None:
                ...
                test_results["success_rate_hierarchical_agent_from_reset_env"][name_env][0].extend(successs_)
                test_results["length_hierarchical_agent_from_reset_env"][name_env][0].extend(lengths_n)
                test_results["interaction_length_hierarchical_agent_from_reset_env"][name_env][0].extend(interaction_lengths_n)
                test_results["subgoal_instructions_dict"][name_env].append(subgoal_instructions_dict)
