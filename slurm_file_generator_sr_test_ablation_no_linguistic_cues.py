...
def get_slurm_script(k):
    return f'''#!/bin/bash
#SBATCH --job-name=DLP_textcrafter_ablation_no_linguistic_cues_gtt_to_pf_unsloth_ll_sr_update_256_lr_HL_1e5_test_sr_epoch_{k}_seed_%a # job name
#SBATCH --time=03:30:00   # maximum execution time (HH:MM:SS)
#SBATCH --output=YOUR_FOLDER/slurm_logs/DLP_textcrafter_ablation_no_linguistic_cues_gtt_to_pf_unsloth_ll_sr_update_256_lr_HL_1e5_test_sr_epoch_{k}_seed_%a.out # output
#SBATCH --error=YOUR_FOLDER/slurm_logs/DLP_textcrafter_ablation_no_linguistic_cues_gtt_to_pf_unsloth_ll_sr_update_256_lr_HL_1e5_test_sr_epoch_{k}_seed_%a.err # err
...
srun slurm/launcher.sh \
                    ...
                    hydra.run.dir=YOUR_FOLDER/hydra_file/DLP_textcrafter_ablation_no_linguistic_cues_gtt_to_pf_unsloth_ll_sr_update_256_lr_HL_1e5_test_sr_epoch_{k}/seed_${{SLURM_ARRAY_TASK_ID}} \
                    rl_script_args.name_experiment=DLP_textcrafter_ablation_no_linguistic_cues_gtt_to_pf_unsloth_ll_sr_update_256_lr_HL_1e5 \
...
for k in range(13, 24):
    with open(f'YOUR_FOLDER/DLP/test_sr/DLP_textcrafter_ablation_no_linguistic_cues_gtt_to_pf_unsloth_ll_sr_update_256_lr_HL_1e5_test_sr_epoch_{k}.slurm', 'w') as f:
        f.write(get_slurm_script(k))
    subprocess.run(['sbatch', f'YOUR_FOLDER/DLP/test_sr/DLP_textcrafter_ablation_no_linguistic_cues_gtt_to_pf_unsloth_ll_sr_update_256_lr_HL_1e5_test_sr_epoch_{k}.slurm'])
    n_runs += 1
