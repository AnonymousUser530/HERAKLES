...
def get_slurm_script(k, nbr_compo):
    return f'''#!/bin/bash
#SBATCH --job-name=DLP_textcrafter_ablation_HL_only_gtt_to_pf_unsloth_test_generalisation_compo_{nbr_compo}_sr_epoch_{k}_seed_%a # job name
#SBATCH --time=02:00:00   # maximum execution time (HH:MM:SS)
#SBATCH --output=YOUR_FOLDER/slurm_logs/DLP_textcrafter_ablation_HL_only_gtt_to_pf_unsloth_test_generalisation_compo_{nbr_compo}_sr_epoch_{k}_seed_%a.out # output
#SBATCH --error=YOUR_FOLDER/slurm_logs/DLP_textcrafter_ablation_HL_only_gtt_to_pf_unsloth_test_generalisation_compo_{nbr_compo}_sr_epoch_{k}_seed_%a.err # err
...
srun slurm/launcher.sh \
                    rl_script_args.path=YOUR_FOLDER/DLP/test_hierarchical_agent_ablation_HL_only_compositional.py \
                    ...
                    hydra.run.dir=YOUR_FOLDER/hydra_file/DLP_textcrafter_ablation_HL_only_gtt_to_pf_unsloth_test_generalisation_compo_{nbr_compo}_sr_epoch_{k}/seed_${{SLURM_ARRAY_TASK_ID}} \
                    rl_script_args.name_experiment=DLP_textcrafter_ablation_HL_only_gtt_to_pf_unsloth \
...
for k in range(13):
    for nbr_compo in range(2, 5):
        with open(f'YOUR_FOLDER/DLP/test_sr/DLP_textcrafter_ablation_HL_only_gtt_to_pf_unsloth_test_generalisation_compo_{nbr_compo}_sr_epoch_{k}.slurm', 'w') as f:
            f.write(get_slurm_script(k, nbr_compo))
        subprocess.run(['sbatch', f'YOUR_FOLDER/DLP/test_sr/DLP_textcrafter_ablation_HL_only_gtt_to_pf_unsloth_test_generalisation_compo_{nbr_compo}_sr_epoch_{k}.slurm'])
        n_runs += 1
