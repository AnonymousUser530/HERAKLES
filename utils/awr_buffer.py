...
class AWRBuffer:
    ...
    """
    def __init__(self, actor_critic_shared_encoder, use_memory, size, gamma=0.99, lam=0.95, device="cpu"):

        self.use_memory = use_memory
        self.actor_critic_shared_encoder = actor_critic_shared_encoder
        if self.use_memory:
            if self.actor_critic_shared_encoder:
                self.obs_buf = {"image": deque(maxlen=size), "instr": deque(maxlen=size),
                                "memory": deque(maxlen=size)}
            else:
                self.obs_buf = {"image": deque(maxlen=size), "instr": deque(maxlen=size),
    ...
    def load(self, save_pah_low_level):
        ...
        try:
            ...
            with open(save_pah_low_level.replace("last","backup"), 'rb') as f:
                print("/!\ load the back up file for AWR buffer /!\ ")
                data = pickle.load(f)
                self.obs_buf = data["obs"]
                self.act_buf = data["act"]
                self.rew_buf = data["rew"]
    ...
    def save(self, save_path):
        ...
        """
        # copy the file to a backup if the file already exists
        if os.path.exists(save_path):
            shutil.copyfile(save_path, save_path.replace("last","backup"))
            ...
            for k, _ in traj.items():
                if k == "obs":
                    for idx in range(len(traj[k])):
                        ...
                        if self.actor_critic_shared_encoder:
                            if self.use_memory:
                                self.obs_buf["memory"].append(traj[k][idx][1].to("cpu"))
                        else:
                            if self.use_memory:
    ...
    def compute_r_lambda(self, model):
        """
        ...
        for end_traj_idx in range(len(self.end_traj_buf)):
            ...
            if self.end_traj_buf[end_traj_idx] > 0:
                ...
                if self.use_memory:
                    if self.actor_critic_shared_encoder:
                        memory_low_level = torch.cat([self.obs_buf["memory"][idx].to(self.device).unsqueeze(0) for idx in range(counter_start, counter_end)])
                    else:
    ...
    def compute_critic_loss(self, model, critic_batch_idx):
        """
        ...
        if self.use_memory:
            if self.actor_critic_shared_encoder:
                memory_low_level = torch.cat([self.obs_buf["memory"][idx].to(self.device).unsqueeze(0) for idx in batch_index])
            else:
    ...
    def compute_policy_loss(self, model, policy_batch_idx, beta_awr, normalisation=False, tsallis_reg=False, q=None):
        """
...
class AWRBuffer_store_trajectories:
    ...
    def store(self, obs, act, rew, **kwargs):
        ...
        Put every tensor on cpu for memory management reasons
        """
        assert self.ptr < self.max_size  # buffer has to have room so you can store
        for idx_obs, _ in enumerate(obs):
            if idx_obs==0:
                if torch.is_tensor(obs[idx_obs].image):
                    obs[idx_obs].image = obs[idx_obs].image.to("cpu")
                else:
                    obs[idx_obs].image = torch.as_tensor(obs[idx_obs].image, dtype=torch.float32).to("cpu")
                if "instr" in obs[idx_obs]:
                    if torch.is_tensor(obs[idx_obs].instr):
                        obs[idx_obs].instr = obs[idx_obs].instr.to("cpu")
                    else:
                        obs[idx_obs].instr = torch.as_tensor(obs[idx_obs].instr, dtype=torch.float32).to("cpu")
            else:
                if torch.is_tensor(obs[idx_obs]):
                    obs[idx_obs] = obs[idx_obs].to("cpu")
                else:
                    obs[idx_obs] = torch.as_tensor(obs[idx_obs], dtype=torch.float32).to("cpu")
