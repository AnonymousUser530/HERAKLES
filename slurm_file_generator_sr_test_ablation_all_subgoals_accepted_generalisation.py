...
def get_slurm_script(k):
    return f'''#!/bin/bash
#SBATCH --job-name=DLP_textcrafter_ablation_all_subgoals_accepted_gtt_to_pf_unsloth_ll_sr_update_256_lr_HL_1e5_test_sr_epoch_{k}_seed_%a # job name
#SBATCH --time=03:30:00   # maximum execution time (HH:MM:SS)
#SBATCH --output=YOUR_FOLDER/slurm_logs/DLP_textcrafter_ablation_all_subgoals_accepted_gtt_to_pf_unsloth_ll_sr_update_256_lr_HL_1e5_test_sr_epoch_{k}_seed_%a.out # output
#SBATCH --error=YOUR_FOLDER/slurm_logs/DLP_textcrafter_ablation_all_subgoals_accepted_gtt_to_pf_unsloth_ll_sr_update_256_lr_HL_1e5_test_sr_epoch_{k}_seed_%a.err # err
...
conda activate /lustre/fsn1/projects/rech/imi/ucy39hi/.conda/envs/lamorel_sandbox_v2
...
srun slurm/launcher.sh \
                    rl_script_args.path=YOUR_FOLDER/DLP/test_hierarchical_agent_ablation_all_subgoals_accepted.py \
                    test_training_script.epoch_k={k} \
                    test_training_script.nbr_tests_hierarchical_agent=40 \
                    test_training_script.goal_space=["go_to_tree","collect_wood","place_table","go_to_table","make_wood_pickaxe","go_to_stone","collect_stone","go_to_coal","collect_coal","place_furnace","go_to_furnace"] \
                    test_training_script.test_name=test_sr \
                    ...
                    hydra.run.dir=YOUR_FOLDER/hydra_file/DLP_textcrafter_ablation_all_subgoals_accepted_gtt_to_pf_unsloth_ll_sr_update_256_lr_HL_1e5_test_sr_epoch_{k}/seed_${{SLURM_ARRAY_TASK_ID}} \
                    rl_script_args.name_experiment=DLP_textcrafter_ablation_all_subgoals_accepted_gtt_to_pf_unsloth_ll_sr_update_256_lr_HL_1e5 \
...
for k in range(10, 13):
    with open(f'YOUR_FOLDER/DLP/test_sr/DLP_textcrafter_ablation_all_subgoals_accepted_gtt_to_pf_unsloth_ll_sr_update_256_lr_HL_1e5_test_sr_epoch_{k}.slurm', 'w') as f:
        f.write(get_slurm_script(k))
    subprocess.run(['sbatch', f'YOUR_FOLDER/DLP/test_sr/DLP_textcrafter_ablation_all_subgoals_accepted_gtt_to_pf_unsloth_ll_sr_update_256_lr_HL_1e5_test_sr_epoch_{k}.slurm'])
    n_runs += 1
