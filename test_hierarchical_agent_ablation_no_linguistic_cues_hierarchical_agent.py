...
def test_hierarchical_agent_sr_from_reset_env(name_env, seed, ll_action_space_id_to_act,
                                              ...
                                              conversion_name_dictionary,
                                              conversion_name_dictionary_reverse):
    ...
    lengths = []
    interaction_lengths = []
    subgoal_instructions_list = []
    # subgoal_length_list = []
    subgoal_instructions_dict = [{tsk: 0 for tsk in task_space} for _ in range(num_envs)]
    # subgoal_length_dict = [{tsk: np.nan for tsk in task_space} for _ in range(num_envs)]

    (obs_env, infos_env), ep_len, ep_interaction_len = (envs.reset(task_space=task_space,
    ...
    lattice = [TokenLattice(tokenizer=tokenizer,
                            valid_actions=["skill {}".format(conversion_name_dictionary[act]) for act in infs["action_space"]]) for infs in infos_env]
    ...
    while len(successs) < nbr_tests:
        ...
        for idx_env in range(num_envs):
            if conversion_name_dictionary_reverse[action_strs[idx_env][0]] not in hl_elementary_actions:
                ...
                subgoal_instructions_dict[idx_env][sub_goal_instructions[idx_env]] += 1
        ...
        for i in range(num_envs):
            if d[i]:
                ...
                interaction_lengths.append(ep_interaction_len[i])
                subgoal_instructions_list.append(subgoal_instructions_dict[i])
                subgoal_instructions_dict[i] = {tsk: 0 for tsk in task_space}
                ep_len[i] = 0
                ep_interaction_len[i] = 0

    successs_arr = np.array(successs[:nbr_tests])
    lengths_arr = np.array(lengths[:nbr_tests])
    interaction_lengths_arr = np.array(interaction_lengths[:nbr_tests])

    subgoal_instructions_list = subgoal_instructions_list[:nbr_tests]
    subgoal_instructions_final = {k: 0 for k in task_space}
    for subgoal_instructions in subgoal_instructions_list:
        for k, v in subgoal_instructions.items():
            subgoal_instructions_final[k] += v/nbr_tests
    ...
    print("interaction length average: {}".format(np.average(interaction_lengths_arr)))
    print("subgoal instructions usage ratio: {}".format(subgoal_instructions_final))

    return successs, lengths, interaction_lengths, subgoal_instructions_final
...
def main(config_args):
    ...
    test_results = {"success_rate_hierarchical_agent_from_reset_env": dict(),
                    ...
                    "interaction_length_hierarchical_agent_from_reset_env": dict(),
                    "subgoal_instructions_dict": dict()
                    }
    for name_env in goal_space:
        time_begin_task = time.time()
        print("Testing task: {}".format(name_env))
        successs_, lengths_n, interaction_lengths_n, subgoal_instructions_dict = test_hierarchical_agent_sr_from_reset_env(name_env=name_env,
                                                                                 seed=seed,
                                                                                 ...
                                                                                 conversion_name_dictionary=conversion_name_dictionary,
                                                                                 conversion_name_dictionary_reverse=conversion_name_dictionary_reverse)
        ...
        test_results["interaction_length_hierarchical_agent_from_reset_env"][name_env] = [interaction_lengths_n]
