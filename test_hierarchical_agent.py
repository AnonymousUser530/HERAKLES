...
def test_hierarchical_agent_sr_from_reset_env(name_env, seed, ll_action_space_id_to_act,
    ...
    elementary_actions_list = []
    # subgoal_length_list = []
    elementary_actions_dict = [{k.replace(" ", "_"): 0 for k in hl_elementary_actions} for _ in range(num_envs)]
    ...
    while len(successs) < nbr_tests:
        ...
        for idx_env in range(num_envs):
            if action_strs[idx_env][0] not in hl_elementary_actions:
                ...
                # the hl agent has chosen an elementary action
                elementary_actions_dict[idx_env][action_strs[idx_env][0].replace(" ", "_")] += 1
        ...
        for i in range(num_envs):
            if d[i]:
                ...
                elementary_actions_list.append(elementary_actions_dict[i])
                elementary_actions_dict[i] = {k.replace(" ", "_"): 0 for k in hl_elementary_actions}
                ep_len[i] = 0
    ...
    elementary_actions_list = elementary_actions_list[:nbr_tests]
    elementary_actions_final = {k.replace(" ", "_"): 0 for k in hl_elementary_actions}
    for elementary_actions in elementary_actions_list:
        for k, v in elementary_actions.items():
            elementary_actions_final[k] += v/nbr_tests
    ...
    print("elementary actions usage ratio: {}".format(elementary_actions_final))

    return successs, lengths, interaction_lengths, subgoal_instructions_final, elementary_actions_final
...
def main(config_args):
    ...
    test_results = {"success_rate_hierarchical_agent_from_reset_env": dict(),
                    ...
                    "elementary_actions_dict": dict(),
                    }
    for name_env in goal_space:
        time_begin_task = time.time()
        print("Testing task: {}".format(name_env))
        successs_, lengths_n, interaction_lengths_n, subgoal_instructions_dict, elementary_actions_final = test_hierarchical_agent_sr_from_reset_env(name_env=name_env,
                                                                                 seed=seed,
                                                                                 ...
                                                                                 device=device,
                                                                                 nbr_tests=config_args.test_training_script.nbr_tests_hierarchical_agent,
                                                                                 rl_script_args=config_args.rl_script_args,
                                                                                 task_space=task_space,
        ...
        test_results["elementary_actions_dict"][name_env] = [elementary_actions_final]
