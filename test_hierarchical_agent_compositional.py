...
def main(config_args):
    ...
    nbr_compo = config_args.test_training_script.test_generalisation.nbr_compositional_tasks
    plural_exceptions = {
      'coal': 'coals',
      'diamond': 'diamonds',
      'drink': 'drinks',
    ...
    for name_env in task_space:
        ...
        if nbr_compo > 1:
            if len(splits) > 2:
                plural_item = plural_exceptions.get(splits[-2]+" "+splits[-1], splits[-2]+" "+splits[-1] + 's')
                name_env_compo = splits[0] + f"_{nbr_compo}_" + plural_item.replace(" ", "_")
            else:
                plural_item = plural_exceptions.get(splits[-1], splits[-1] + 's')
                name_env_compo = name_env.replace(splits[-1], f"{nbr_compo}_"+plural_item)
        else:
            name_env_compo = name_env

        print("\nTesting task: {}".format(name_env_compo))
        successs_, lengths_n, interaction_lengths_n, subgoal_instructions_dict = test_hierarchical_agent_sr_from_reset_env(name_env=name_env_compo,
                                                                                     seed=seed,
