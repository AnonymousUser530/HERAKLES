...
def main(config_args):
    ...
    conversion_name_dictionary = {k: "{}".format(v) for k, v in ll_action_space_act_to_id.items()}
    for id_tsk, tsk in enumerate(task_space):
        conversion_name_dictionary[tsk.replace("_", " ")] = "{}".format(id_tsk + len(ll_action_space_act_to_id))
    conversion_name_dictionary_reverse = {"skill {}".format(v): k for k, v in conversion_name_dictionary.items()}

    print("conversion_name_dictionary: ", conversion_name_dictionary)
    print("conversion_name_dictionary_reverse: ", conversion_name_dictionary_reverse)
    ...
    lattice = [TokenLattice(tokenizer=tokenizer,
                            valid_actions=["skill {}".format(conversion_name_dictionary[act]) for act in infs["action_space"]]) for infs in infos]
    ...
    for epoch in range(begin_epoch, config_args.rl_script_args.epochs):
        ...
        while steps_counter < (config_args.rl_script_args.steps_per_epoch // num_envs) and token_generated < config_args.rl_script_args.max_tokens_per_epoch:
            ...
            for idx_env in range(num_envs):
                if conversion_name_dictionary_reverse[action_strs[idx_env][0]] not in hl_elementary_actions:
                    # the hl agent has chosen a subgoal to be done by the ll agent
                    sub_goal_instructions[idx_env]=conversion_name_dictionary_reverse[action_strs[idx_env][0]].replace(" ", "_")
            """
            ...
            while not all(envs.get_stop_mask()):
                ...
                for a_j in range(num_envs):
                    if sub_goal_instructions[a_j] is None and not old_stop_mask[a_j]:
                        actions_id[a_j] = conversion_name_dictionary_reverse[action_strs[a_j][0]]
                ...
                for idx, (new_o, new_d, new_info) in enumerate(zip(return_o, return_d, return_infos)):
                    if not old_stop_mask[idx]:
                        ...
                        if new_d:
                            lattice[idx] = TokenLattice(tokenizer=tokenizer,
                                                        valid_actions=["skill {}".format(conversion_name_dictionary[act]) for act in new_info["action_space"]])
            ...
            if len(bootstrap_dict["ids"]) > 0:
                ...
                for _i in range(len(output)):
                    try:
                        if len(output[_i]["value"]["value"].squeeze().shape) == 0:
                            buffers[bootstrap_dict["ids"][_i]].finish_path(
                                output[_i]["value"]["value"].squeeze().item())
                        else:
                        ...
                        print("_i: {}".format(_i))
                        print("output[_i][value][value]: {}".format(output[_i]["value"]["value"]))
                        print("output[_i][value][value].size(): {}".format(output[_i]["value"]["value"].size()))
                        print("output[_i][value][model_head].size(): {}".format(output[_i]["value"]["model_head"].size()))
                        raise Exception("Error in bootstrap value target")
        ...
        if config_args.rl_script_args.test_hierarchical_agent and (epoch % config_args.rl_script_args.test_hierarchical_agent_freq == 0 or epoch == config_args.rl_script_args.epochs - 1):
        ...
        """
        # save new examples from the buffer used to train MAGELLAN
        if len(buffer_hl_sr_estimator["obs"][-1]) > 0:
            buffer_training_MAGELLAN = f"{config_args.rl_script_args.output_dir}/{name_experiment}/buffer_training_MAGELLAN_seed_{config_args.rl_script_args.seed}_last.pkl"
            if os.path.exists(buffer_training_MAGELLAN):
                shutil.copyfile(buffer_training_MAGELLAN, buffer_training_MAGELLAN.replace("last","backup"))
                with open(buffer_training_MAGELLAN, 'rb') as file:
                    dict_history_buffer_MAGELLAN_trainer = pickle.load(file)
                dict_history_buffer_MAGELLAN_trainer["obs"].append(buffer_hl_sr_estimator["obs"][-1])
                dict_history_buffer_MAGELLAN_trainer["success"].append(buffer_hl_sr_estimator["success"][-1])
                with open(buffer_training_MAGELLAN, 'wb') as file:
                    pickle.dump(dict_history_buffer_MAGELLAN_trainer, file)
                del dict_history_buffer_MAGELLAN_trainer
            else:
                dict_history_buffer_MAGELLAN_trainer = dict()
                dict_history_buffer_MAGELLAN_trainer["obs"] = [buffer_hl_sr_estimator["obs"][-1]]
                dict_history_buffer_MAGELLAN_trainer["success"] = [buffer_hl_sr_estimator["success"][-1]]

                with open(buffer_training_MAGELLAN, 'wb') as file:
                    pickle.dump(dict_history_buffer_MAGELLAN_trainer, file)
                del dict_history_buffer_MAGELLAN_trainer

        # save new examples from the buffer used to train ll_sr_estimator
        if len(buffer_ll_sr_estimator["obs"][-1]) > 0:
            buffer_training_ll_sr_estimator = f"{config_args.rl_script_args.output_dir}/{name_experiment}/buffer_training_ll_sr_estimator_seed_{config_args.rl_script_args.seed}_last.pkl"
            if os.path.exists(buffer_training_ll_sr_estimator):
                shutil.copyfile(buffer_training_ll_sr_estimator, buffer_training_ll_sr_estimator.replace("last","backup"))
                with open(buffer_training_ll_sr_estimator, 'rb') as file:
                    dict_history_buffer_ll_sr_estimator_trainer = pickle.load(file)
                dict_history_buffer_ll_sr_estimator_trainer["obs"].append(buffer_ll_sr_estimator["obs"][-1])
                dict_history_buffer_ll_sr_estimator_trainer["success"].append(buffer_ll_sr_estimator["success"][-1])
                with open(buffer_training_ll_sr_estimator, 'wb') as file:
                    pickle.dump(dict_history_buffer_ll_sr_estimator_trainer, file)
                del dict_history_buffer_ll_sr_estimator_trainer
            else:
                dict_history_buffer_ll_sr_estimator_trainer = dict()
                dict_history_buffer_ll_sr_estimator_trainer["obs"] = [buffer_ll_sr_estimator["obs"][-1]]
                dict_history_buffer_ll_sr_estimator_trainer["success"] = [buffer_ll_sr_estimator["success"][-1]]
        ...
        print("update_results: {}".format(update_results))
